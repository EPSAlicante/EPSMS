...
def getPackageManager():
    ...
    packageManager = subprocess.Popen("((%s --version >/dev/null && echo 'rpm') || (%s --version >/dev/null && echo 'conary') || (%s --version >/dev/null && echo 'dpkg') || (%s --version >/dev/null && echo 'emerge') || (%s --version >/dev/null && echo 'pacman') || (%s >/dev/null && echo 'pkgtools') || (%s >/dev/null && echo 'pkgutil') || (%s --version >/dev/null && echo 'zypper') || (%s >/dev/null && echo 'installpkg') ||  echo 'unknown') 2>%s" % (path('rpm'), path('conary'), path('dpkg-query'), path('equery'), path('pacman'), path('pkg_info'), path('pkgutil'), path('zypper'), path('installpkg'), errorLog), shell=True, executable='%s' % (bash), stdout=subprocess.PIPE).stdout.read().strip()
...
def show_packages(type):
    ...
    if type == 'rpm':
      ...
      command = "%s -as|%s -v '^$' |%s -v '^Package'|%s -v '^Total size'|%s 'BEGIN {RS=\"\"}{gsub(/:\\n/,\" \",$0); print $0}'|tr '\t' ' '|tr -s ' '|cut -d' ' -f3,4|rev|%s 's/-/ /'|rev|%s 's/^.*://'|%s -v '^$'" % (path('pkg_info'), grep, grep, grep, awk, sed, sed, grep)

    elif type == 'pkgutil':
      command = "%s -A|tr -s ' '|%s -v '^$'|%s -v '^Package'|%s -v 'not installed'|cut -d',' -f1|sort" % (path('pkgutil'), grep, grep, grep)
